            ...
            for key in list(state_dict.keys()):
                if key in model_state_dict:
                    if state_dict[key].shape != model_state_dict[key].shape:
                        # 如果 checkpoint 中的参数比模型需要的大，进行切片
                        if state_dict[key].shape[0] > model_state_dict[key].shape[0]:
                            logging.warning(
                                f"Slicing parameter {key} from shape {state_dict[key].shape} to {model_state_dict[key].shape}"
                            )
                            state_dict[key] = state_dict[key][:model_state_dict[key].shape[0]]
